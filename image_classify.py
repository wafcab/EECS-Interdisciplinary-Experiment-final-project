...
from toESP_Arduino import ESP_send
...
def run(model: str, max_results: int, num_threads: int, enable_edgetpu: bool,
  ...
  while cap.isOpened():
    ...
    for idx, category in enumerate(categories):
      ...
      '''@@ Send the result of image classifying to ESP/Arduino
      Choose the one having the highest score as the input'''
      #@ ouput the highest one as the result
      if idx == 0 :
        image_output = class_name
        print(image_output)
        ESP_send(image_output)
    ...
    key_pressing = cv2.waitKey(1) & 0xFF
    ...
    if key_pressing == 27:
      ...
      return 'ESC'
    #@ Change the input source to Audio classifying if the 'A' key is pressed.
    elif key_pressing == ord('a'):
      #ESP_close()
      cap.release()
      cv2.destroyAllWindows()
      return 'Audio'
